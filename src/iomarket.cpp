// Copyright 2022 The Forgotten Server Authors. All rights reserved.static_cast<int>(action)static_cast<MarketActio
...
MarketOfferList IOMarket::getActiveOffers(MarketAction_t action, uint16_t itemId)
{
	...
	DBResult_ptr result = Database::getInstance()->storeQuery(fmt::format("SELECT `id`, `amount`, `price`, `created`, `anonymous`, (SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` FROM `market_offers` WHERE  AND `itemtype` = {:d}",  itemId));
	if (!result) {
		return offerList;
	}
...
MarketOfferList IOMarket::getOwnOffers(MarketAction_t action, uint32_t playerId)
{
	...
	DBResult_ptr result = Database::getInstance()->storeQuery(fmt::format("SELECT `id`, `amount`, `price`, `created`, `itemtype` FROM `market_offers` WHERE `player_id` = {:d} ", playerId));
	if (!result) {
		return offerList;
	}
...
HistoryMarketOfferList IOMarket::getOwnHistory(MarketAction_t action, uint32_t playerId)
{
	...
	DBResult_ptr result = Database::getInstance()->storeQuery(fmt::format("SELECT `itemtype`, `amount`, `price`, `expires_at`, `state` FROM `market_history` WHERE `player_id` = {:d} ", playerId));
	if (!result) {
		return offerList;
	}
...
{
	Database::getInstance()->executeQuery(fmt::format("INSERT INTO `market_offers` (`player_id`, `sale`, `itemtype`, `amount`, `price`, `created`, `anonymous`) VALUES ({:d}, 0, {:d}, {:d}, {:d}, {:d}, {:d})", playerId,  itemId, amount, price, time(nullptr), anonymous));
}
...
{
	g_databaseTasks.addTask(fmt::format("INSERT INTO `market_history` (`player_id`, `sale`, `itemtype`, `amount`, `price`, `expires_at`, `inserted`, `state`) VALUES ({:d}, 0, {:d}, {:d}, {:d}, {:d}, {:d})", playerId,  itemId, amount, price, timestamp, time(nullptr) ));
}
...
{
	DBResult_ptr result = Database::getInstance()->storeQuery(fmt::format("SELECT `sale` AS `sale`, `itemtype` AS `itemtype`, COUNT(`price`) AS `num`, MIN(`price`) AS `min`, MAX(`price`) AS `max`, SUM(`price`) AS `sum` FROM `market_history`  GROUP BY `itemtype`, `sale`"));
	if (!result) {
		return;
	}
