...
bool IOMapSerialize::saveHouseItems()
{
	...
	for (const auto& it : g_game.map.houses.getHouses()) {
		...
		for (HouseTile* tile : house->getTiles()) {
			...
			const char* attributes1 = stream.getStream(attributesSize);
			if (attributesSize > 0) {
				query << house->getId() << ',' << db->escapeBlob(attributes1, attributesSize);
...
bool IOMapSerialize::saveHouseInfo()
{
	...
	for (const auto& it : g_game.map.houses.getHouses()) {
		...
		if (house->getAccessList(GUEST_LIST, listText) && !listText.empty()) {
			query << house->getId() << ',' << fmt::underlying(GUEST_LIST) << ',' << db->escapeString(listText);
			if (!stmt.addRow(query)) {
				return false;
			}
		...
		if (house->getAccessList(SUBOWNER_LIST, listText) && !listText.empty()) {
			query << house->getId() << ',' << fmt::underlying(SUBOWNER_LIST) << ',' << db->escapeString(listText);
			if (!stmt.addRow(query)) {
				return false;
			}
